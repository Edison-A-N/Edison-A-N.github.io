...
### 6. Agent-as-Tool 的演进历史

2026 年面试里还可以主动补一个判断：**Agent-as-Tool 不是“多几个 agent 一起聊天”，而是 tool calling 继续演化后的编排模式。**

它的大致演进路径可以这样理解：

| 阶段 | 代表形态 | 核心变化 |
| --- | --- | --- |
| ReAct / Agent Loop | Reasoning → Action → Observation | LLM 开始根据观察结果动态决定下一步动作 |
| Function / Tool Calling | OpenAI function calling、Anthropic tool use | 动作从自然语言变成结构化 API 调用 |
| Multi-Agent 编排 | AutoGen、CrewAI、LangGraph workflow | 多个 agent 开始按角色协作，但状态、上下文和控制权更复杂 |
| Agent-as-Tool / Subagent | OpenAI `agent.asTool()`、AutoGen `AgentTool`、Claude Code Agent tool、DeepAgents subagents | 专才 agent 被包装成可调用工具，由主控 agent 保留对话和决策控制权 |
| Harness 化 | DeepAgents、coding agent、MCP ecosystem | agent、tool、skill、权限、文件系统、trace、eval 被放进统一运行时约束里 |

这条路径背后的核心动机不是“让系统看起来更智能”，而是解决三个工程问题：

1. **上下文膨胀**：复杂子任务会产生大量中间工具调用和观察结果，如果全塞回主 agent，上下文会变脏、变贵、变难控。
2. **职责隔离**：代码审查、搜索、数据分析、测试生成这类任务可以交给专才 agent；主 agent 只拿最终结构化结果。
3. **控制权收敛**：和 handoff 不同，agent-as-tool 通常不把用户对话完全交出去，而是由 supervisor / manager agent 调用子 agent，再决定如何回复用户。

所以，agent-as-tool 更像“把一个小型 agent loop 封装成高级工具”，而不是“几个 agent 平权聊天”。这也是为什么 OpenAI Agents SDK 会区分 agents-as-tools 和 handoffs，LangChain / DeepAgents 会强调 subagent 用来缓解 context bloat，Claude Code 的 Agent tool 会让子 agent 在独立 context window 中运行，只把最终结果返回给父会话。

但它也不是免费午餐。把 agent 当 tool 后，tool 的不确定性变高了：普通函数通常输入输出清晰，而子 agent 可能会继续调用工具、消耗 token、访问文件、触发权限边界，甚至产生自己的失败循环。因此生产系统必须把 subagent 当成“高风险工具”治理：

- input / output schema 要更严格，最好返回结构化 artifact，而不是一大段解释；
- 要有 timeout、token budget、最大工具调用次数和取消机制；
- 要隔离上下文、文件系统和权限，避免子 agent 继承过多父级能力；
- trace 里要能看到子 agent 做了什么，否则最终答案对了也无法审计 trajectory；
- 并发执行要谨慎，尤其是带内部状态或共享 workspace 的 agent。

面试里可以这样总结：

> Agent-as-Tool 是 tool calling 的高阶形态：把一个专才 agent 封装成可调用能力，由主 agent 保留控制权。它的价值是隔离上下文、复用专长和控制复杂度；它的风险是把普通工具的不确定性放大成一个子系统，所以必须配套 schema、预算、权限、trace、超时和 eval。生产里我会优先把它放进 deterministic workflow 或 harness 里，而不是让多个 agent 自由聊天。
...
- OpenAI Agents as tools: https://developers.openai.com/api/docs/guides/agents/define-agents
...
- AutoGen AgentTool: https://microsoft.github.io/autogen/stable/reference/python/autogen_agentchat.tools.html
- Microsoft Agent Framework: https://github.com/microsoft/agent-framework
- Claude Code subagents: https://docs.anthropic.com/en/docs/claude-code/sub-agents
- Claude Code Agent SDK subagents: https://docs.claude.com/en/docs/claude-code/sdk/subagents
