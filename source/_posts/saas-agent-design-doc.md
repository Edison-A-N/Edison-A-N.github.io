...
> 资料时效性说明：Agent 领域变化很快，本文按 **2026-05-14** 可访问的公开资料整理，不会把早期 agent loop / tool use 文档当成最新生产架构结论。文中把资料分成三类使用：第一类是 **2025 年 10 月以后到 2026 年的最新工程资料**，例如 Salesforce Agent Script / Agent Graph、Microsoft Agent Framework Workflows / Durable Agents、Oracle AI Agent Studio Workflow 等；第二类是仍在更新的官方产品文档，例如 Dify、Intercom Fin、CrewAI、LangGraph；第三类是 OpenAI / Anthropic tool use 这类基础机制资料，主要用于解释工具调用原语，而不是直接证明生产级 SaaS Agent 的架构方向。没有明确发布日期的 live docs，只按“截至本文写作时的公开页面状态”使用。

## 一、从公开资料和工程实践归纳出的设计原则

### 1. 先 workflow，后 autonomy

这个判断不是从“Agent 应该更保守”这个直觉出发，而是从公开官方资料、产品能力和工程实践里反复出现的同一个矛盾归纳出来的：**模型擅长处理模糊输入，但生产系统需要可重复、可审计、可恢复、可测试的执行路径。**

不同厂商和框架的表述不完全一样，但指向的设计取向很接近。下面的“原文摘录”保留英文原句，方便追踪来源；右侧才是本文的推导。

| 来源 | 公开资料原文摘录 | 对 SaaS Agent 的推导 |
| --- | --- | --- |
| [Salesforce Agentforce hybrid reasoning](https://architect.salesforce.com/docs/architect/fundamentals/guide/hybrid-reasoning-agentforce-builder-agent-script.html) / [Agent Script control plane](https://www.salesforce.com/blog/agent-script-control-plane/)（2025-10 到 2026-04） | “separating deterministic execution from LLM reasoning”；“if you can express a decision as code, it should be written as logic”；“Agent Script lets you set control per decision.” | 这是对 workflow-first 最直接的工程实践支撑之一：先定义可执行结构和确定性决策，再在需要判断、解释、自然语言生成的节点调用 LLM |
| [Dify Workflow & Chatflow](https://docs.dify.ai/en/use-dify/build/workflow-chatflow) | “AI models are powerful, but on their own they can be unpredictable”；“you design a flow that orchestrates models, tools, and logic step by step—with clear conditions, checkpoints, and fallback paths.” | workflow 是降低幻觉、漏步骤和不一致输出的产品化手段 |
| [Microsoft Agent Framework Workflows](https://learn.microsoft.com/en-us/agent-framework/journey/workflows) / [Overview](https://learn.microsoft.com/en-us/agent-framework/overview/)（2026-04） | “the LLM decides what happens next”；“that’s powerful for open-ended tasks... but it’s a liability when the process itself has rules”；“Use a workflow when... The process has well-defined steps / You need explicit control over execution order.” | 先判断任务是否能确定性表达；能确定的部分不应交给 agent 自由规划 |
| [Intercom Fin Procedures](https://fin.ai/procedures) | “Give Fin detailed, step-by-step instructions”；“Fin combines agentic behavior with deterministic control”；“Turn written rules into enforced logic.” | 面向业务团队的 Agent 产品也在把流程、规则、测试和升级路径显式化 |
| [Oracle AI Agent Studio Workflow](https://docs.oracle.com/en/cloud/saas/fusion-ai/aiaas/create-custom-ai-agents-of-type-workflow.html) | “Agent teams of type Workflow consist of a sequence of nodes used for deterministic, rule-based orchestration”；“Workflows are ideal for scenarios where compliance, repeatability, and governance are essential.” | 企业 SaaS 厂商也在把 Agent 拆成节点、条件、业务函数、LLM 节点和输出 schema |
| [LangGraph overview](https://docs.langchain.com/oss/python/langgraph) / [CrewAI Flows](https://docs.crewai.com/en/concepts/flows.md) | LangGraph: “orchestration framework and runtime for building, managing, and deploying long-running, stateful agents”；“durable execution, streaming, human-in-the-loop”；CrewAI: “structured, event-driven workflows”；“manage state”；“Flexible Control Flow” | 这是偏框架层的间接证据：生产 Agent 需要运行时编排、状态和恢复能力，而不是只靠一次聊天循环 |
| [Google Gemini Enterprise Agent Platform overview](https://docs.cloud.google.com/gemini-enterprise-agent-platform/overview) | “build, deploy, govern, and optimize enterprise-grade AI agents”；“Agent Registry”；“Agent Identity”；“Agent Gateway”；“Agent evaluation”；“Observability.” | 作为平台趋势信号：最新企业 Agent 已经从“能调用工具”扩展到全生命周期治理、评估和运行时管理 |
| [OpenAI Agents SDK](https://developers.openai.com/api/docs/guides/agents) / [Anthropic Claude tool use](https://docs.claude.com/en/docs/agents-and-tools/tool-use/overview) | OpenAI: running agents、handoff、sessions、traces；Anthropic: tool schema、`tool_use` / `tool_result`、tool execution model | 它们说明“模型如何调用工具”，但不足以单独推出“生产 SaaS 应该高度自治”；SaaS 侧还必须补 workflow、权限、审批和审计 |

所以本文里的“先 workflow，后 autonomy”更准确地说是：**先把可确定的业务路径、状态和副作用边界显式建模，再把 LLM 放进无法完全规则化的局部节点。** 对多数高风险、长流程 SaaS 场景，更稳妥的设计不是“让 agent 全自动决定一切”，而是：
...
这里要避免过度解读：不是所有资料都直接说“必须先 workflow”。OpenAI / Anthropic 这类模型厂商文档更多是在定义工具调用和 agent loop；LangGraph / CrewAI 这类框架更多是在提供状态、编排和恢复能力；Salesforce、Dify、Microsoft、Intercom、Oracle 的资料则更直接把确定性流程、显式执行顺序、测试、审批和可审计性放在生产 Agent 的核心位置。把这些材料合在一起，才形成本文的设计判断：**workflow 负责确定性，LLM 负责不确定性。**（来源：[LangGraph overview](https://docs.langchain.com/oss/python/langgraph)、[Microsoft Agent Framework Overview](https://learn.microsoft.com/en-us/agent-framework/overview/)、[Microsoft Agent Framework Workflows](https://learn.microsoft.com/en-us/agent-framework/journey/workflows)、[Salesforce Agentforce hybrid reasoning architecture](https://architect.salesforce.com/docs/architect/fundamentals/guide/hybrid-reasoning-agentforce-builder-agent-script.html)、[Salesforce Blog - Agent Script control plane](https://www.salesforce.com/blog/agent-script-control-plane/)、[Dify Workflow & Chatflow](https://docs.dify.ai/en/use-dify/build/workflow-chatflow)、[Intercom Fin Procedures](https://fin.ai/procedures)、[Oracle AI Agent Studio Workflow](https://docs.oracle.com/en/cloud/saas/fusion-ai/aiaas/create-custom-ai-agents-of-type-workflow.html)、[CrewAI Flows](https://docs.crewai.com/en/concepts/flows.md)）
...
这一类资料是 Agent 系统的基础层证据：它告诉我们模型如何参与工具调用循环，但它本身并不解决业务流程的确定性、审批、回滚和审计。因此，在 SaaS 场景里，标准 agent loop 更适合作为 workflow 节点里的执行机制，而不是整个产品架构的唯一抽象。
...
这对 SaaS Agent 非常关键。LangGraph 的证据不是“所有 Agent 都必须先画流程图”，而是：一旦任务变成长时间、有状态、可能失败或需要人工介入的执行过程，系统就需要 workflow runtime 级别的能力。因为 SaaS Agent 往往不是一次请求就结束，而是会经历：
...
### 3. Microsoft Agent Framework：2026 年的 workflow / durable agent 取向

Microsoft Agent Framework / Semantic Kernel 的公开资料强调以下企业级 agent 能力。尤其是 2026 年 4 月更新的 Workflows 文档，已经不是泛泛地说“支持多 Agent”，而是明确讨论什么时候不能让 LLM 决定下一步。（来源：[Microsoft Agent Framework Overview](https://learn.microsoft.com/en-us/agent-framework/overview/)、[Microsoft Agent Framework Workflows](https://learn.microsoft.com/en-us/agent-framework/journey/workflows)、[Microsoft Agent Framework Workflows Overview](https://learn.microsoft.com/en-us/agent-framework/user-guide/workflows/overview)）
...
Microsoft 的资料对 workflow-first 的支持比较直接：它明确把“open-ended or conversational”归给 agent，把“well-defined steps、explicit execution order、checkpointing、human-in-the-loop”归给 workflow；Workflows 文档进一步说明，前面那些 agent patterns 的共同点是 **LLM decides what happens next**，这在开放任务里有用，但当流程本身有规则时会成为 liability。它给出的折中方案是：workflow 定义结构、顺序和 gate，agent executor 只处理真正需要 LLM 推理的步骤。

2026 年的 Durable Agents / Azure Functions 集成又把这个方向推进了一步：它强调 stateful agents、deterministic multi-agent orchestrations、failure resume、human-in-the-loop pause / resume 和可观测 dashboard。这个更新说明生产 Agent 的问题已经从“模型会不会调用工具”推进到“长任务如何可靠执行、失败后如何恢复、人审等待期间如何保存状态和控制成本”。（来源：[Microsoft Agent Framework Durable Agents](https://learn.microsoft.com/en-us/agent-framework/integrations/azure-functions)）
...
它说明客服 Agent 的关键不是“让模型自由聊天”，而是把业务流程、知识、连接器、测试、监控和人工升级组合成闭环。Fin Procedures 对 workflow-first 的支撑也很直接：复杂 query 被拆成 step-by-step instructions、if/else、code-enforced rules 和 Simulations，而不是只靠一个通用客服 prompt。

### 5. Salesforce Agentforce：确定性逻辑与 LLM 推理混合

Salesforce Agentforce 是企业 Agent 设计里非常值得单独研究的案例，而且它的关键材料集中出现在 2025 年 10 月以后：2025 年 10 月 Salesforce Developers Blog 介绍 Agent Script 和 hybrid reasoning，2025 年 10 月 Salesforce Engineering 文章解释 Agent Graph 和 guided determinism，2026 年 2 月 Agent Script decoded 进一步拆解语言基础，2026 年 4 月 control plane 文章又把 Agent Script 描述成 agentic decisions 的控制平面。这些资料共同传递一个信号：**企业 Agent 不能只靠 prompt 驱动，而要把可确定的业务逻辑、权限边界、测试追踪和治理能力产品化。**（来源：[Salesforce Developers Blog - Introducing Hybrid Reasoning with Agent Script](https://developer.salesforce.com/blogs/2025/10/introducing-hybrid-reasoning-with-agent-script)、[Salesforce Engineering - Agentforce Agent Graph](https://engineering.salesforce.com/agentforces-agent-graph-toward-guided-determinism-with-hybrid-reasoning/)、[Salesforce Developers Blog - Agent Script decoded](https://developer.salesforce.com/blogs/2026/02/agent-script-decoded-intro-to-agent-script-language-fundamentals)、[Salesforce Blog - Agent Script control plane](https://www.salesforce.com/blog/agent-script-control-plane/)）
...
### 6. Oracle / Google：企业 Agent 平台也在走向 workflow、身份和治理

2026 年以后，企业软件厂商的 Agent 平台资料里也出现了同样方向。Oracle AI Agent Studio 的 Workflow 类型 agent team 被定义为“deterministic, rule-based orchestration”，强调每一步预配置，适合 compliance、repeatability、governance；示例里把采购申请拆成 set variables、business object function、IF conditional、document processor、LLM、send email 等节点，并要求定义 workflow-level variables、output schema、error handling 和 access roles。（来源：[Oracle AI Agent Studio Workflow](https://docs.oracle.com/en/cloud/saas/fusion-ai/aiaas/create-custom-ai-agents-of-type-workflow.html)）

Google Gemini Enterprise Agent Platform 的公开产品资料则体现了另一个新趋势：企业 Agent 平台不再只讨论 agent loop，而是把 Agent Studio、Agent Runtime、Agent Registry、Agent Identity、Agent Gateway、治理策略、评估、仿真和观测放在同一个平台生命周期里。这个来源更适合作为“平台化趋势信号”，而不是 workflow-first 的单点证明。（来源：[Google Gemini Enterprise Agent Platform overview](https://docs.cloud.google.com/gemini-enterprise-agent-platform/overview)）

### 7. Zapier Agents：模板、连接器、治理边界
...
### 8. Relevance AI / Lindy：团队化、治理化、运营化

Relevance AI 和 Lindy 的公开资料都在强调团队化和治理化能力。这里主要把它们作为市场产品信号，而不是像 Salesforce / Microsoft / Oracle 那样作为 workflow-first 的强架构证据；具体能力仍应以其最新 docs、changelog 或企业合同说明为准。（来源：[Relevance AI Docs index](https://relevanceai.com/docs/llms.txt)、[Relevance AI product overview](https://www.relevanceai.com/)、[Lindy Enterprise](https://www.lindy.ai/blog/lindy-enterprise-announcement)、[Lindy 3.0](https://www.lindy.ai/blog/lindy-3-0)）
...
### 9. Dify / CrewAI / AutoGPT / OpenHands：开源实现的工程启发

几个开源生态分别提供了不同启发。这里要特别注意时效性：Dify 和 CrewAI 的文档仍适合作为当前能力参考；AutoGPT / OpenHands 的 GitHub README 更适合作为开源平台化方向的启发，不能替代具体版本或 release notes 来证明 2026 年最新架构。（来源：[Dify Workflow & Chatflow](https://docs.dify.ai/en/use-dify/build/workflow-chatflow)、[Dify Agent](https://docs.dify.ai/en/use-dify/build/agent)、[Dify Tools](https://docs.dify.ai/en/use-dify/workspace/tools)、[CrewAI Flows](https://docs.crewai.com/en/concepts/flows)、[CrewAI AMP](https://docs.crewai.com/en/enterprise/introduction)、[AutoGPT](https://github.com/Significant-Gravitas/AutoGPT)、[OpenHands](https://github.com/OpenHands/OpenHands)）
...
这里的证据强度不完全相同：Dify 对 Workflow / Chatflow / Agent 的区分很直接；CrewAI 同时强调 Crews 和 Flows，更像是在说明“自主协作”和“显式流程”是两类可组合能力；AutoGPT、OpenHands 更多提供平台化、执行记录和治理方向的间接启发。因此本文不会把它们都解读成同一句口号，而是把它们共同归纳为：生产化 Agent 必须有流程、状态、评估和治理层。
...
```mermaid
flowchart TD
  user["User / Admin / Auditor"]
  entry["Web App / API / Webhook / Slack / Email"]
  auth["AuthN → Tenant Resolve → RBAC / ABAC"]

  gateway["Agent Gateway<br/>- quota check<br/>- policy check<br/>- risk classification<br/>- prompt injection detection"]
  workflow["Orchestrator / Workflow Runtime<br/>- state machine<br/>- checkpoint<br/>- retry / timeout<br/>- human interrupt"]

  llm["LLM Runtime<br/>- model routing<br/>- prompt assembly<br/>- context window control<br/>- structured output"]
  tool["Tool Runtime<br/>- tool registry<br/>- schema validation<br/>- permission check<br/>- idempotency<br/>- connector secrets"]
  memory["Memory / RAG<br/>- tenant namespace<br/>- retrieval policy<br/>- memory scope<br/>- delete / export"]
  approval["Approval Queue<br/>- risk explanation<br/>- diff preview<br/>- approver policy<br/>- resume execution"]

  governance["Cross-cutting Governance<br/>Audit Log / Trace / Metrics / Evals<br/>Billing / Cost Control / Admin Console"]

  user --> entry --> auth --> gateway --> workflow
  workflow --> llm
  workflow --> tool
  workflow --> memory
  workflow --> approval
...
flowchart TD
  ...
  resource_permission["Target resource permission"]
  runtime_risk["Runtime risk policy"]
  effective["Effective permission<br/>取所有边界的交集"]

  tenant_policy --> effective
  actor_permission --> effective
  agent_policy --> effective
  tool_scope --> effective
  resource_permission --> effective
  runtime_risk --> effective
```
...
flowchart TD
  start["用户咨询退款"]
  intent["识别意图<br/>退款 / 投诉 / 普通问答"]
  retrieve["检索退款政策和订单信息"]
  decision{"是否满足自动处理条件？"}
  low["低风险：生成回复草稿"]
  high["高风险：创建审批请求"]
  approve{"审批通过？"}
  ...
  start --> intent --> retrieve --> decision
  decision -->|低风险| low --> audit
  decision -->|高风险| high --> approve
  approve -->|是| refund --> audit
  approve -->|否| escalate --> audit
  audit --> notify
```
...
- Microsoft Agent Framework Workflows: https://learn.microsoft.com/en-us/agent-framework/journey/workflows
- Microsoft Agent Framework Workflows Overview: https://learn.microsoft.com/en-us/agent-framework/user-guide/workflows/overview
- Microsoft Agent Framework Durable Agents: https://learn.microsoft.com/en-us/agent-framework/integrations/azure-functions
...
- Salesforce Developers Blog - Introducing Hybrid Reasoning with Agent Script: https://developer.salesforce.com/blogs/2025/10/introducing-hybrid-reasoning-with-agent-script
...
- Oracle AI Agent Studio Workflow: https://docs.oracle.com/en/cloud/saas/fusion-ai/aiaas/create-custom-ai-agents-of-type-workflow.html
